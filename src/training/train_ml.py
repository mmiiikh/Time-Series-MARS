...
from src.config.settings import (
    ...
    LGBM_DEFAULT_PARAMS, XGB_DEFAULT_PARAMS, RF_DEFAULT_PARAMS,
    OPTUNA_N_TRIALS_LGBM, OPTUNA_N_TRIALS_XGB, OPTUNA_N_TRIALS_RF)
from src.data.load_data import load_data, create_series_dict
from src.data.preprocess import analyze_ccf, build_ccf_lag_ranges, plot_ccf_heatmap,build_global_dataset, make_cv_folds
from src.forecasting.ml_model import (
    DirectForecaster, ModelFactory, make_lgbm, make_xgb, make_elasticnet,
    make_rf, make_lgbm_quantile, run_feature_selection, cv_mape_score)
...
def ccf_analysis(series_dict: dict) -> dict:
    series_dict_stationary = {}
    for key, df in series_dict.items():
        df_st = df.copy()
        if TARGET_COL in df_st.columns:
            df_st[TARGET_COL] = df_st[TARGET_COL].diff()
        for col in EXOG_COLS:
            if col in df_st.columns:
                df_st[col] = df_st[col].diff()
        df_st = df_st.dropna(subset=[TARGET_COL])
        series_dict_stationary[key] = df_st
    ccf_df, ccf_summary = analyze_ccf(series_dict_stationary)
    plot_ccf_heatmap(ccf_df, save_path=str(ML_RESULTS_DIR / "ccf_heatmap.png"))
    lag_ranges = build_ccf_lag_ranges(ccf_summary)
    return lag_ranges


def build_dataset(series_dict: dict, lag_ranges: dict) -> tuple:
...
def feature_selection(global_df: pd.DataFrame,feature_cols:list,cv_folds: list) -> list:
    return run_feature_selection(global_df, feature_cols, cv_folds)

def tune_hyperparams(global_df: pd.DataFrame,final_features: list,cv_folds: list) -> dict:
    ...
    def lgbm_obj(trial):
        ...
        params = {
            ...
            "max_depth": md,
            "num_leaves": trial.suggest_int("num_leaves", 2, min(127, 2**md - 1)),
        ...
        return cv_mape_score(global_df, final_features, cv_folds,lambda: lgb.LGBMRegressor(**params), horizons=CV_HORIZONS)

    lgbm_study = optuna.create_study(direction="minimize",sampler=optuna.samplers.TPESampler(seed=RANDOM_STATE))
    lgbm_study.optimize(lgbm_obj, n_trials=OPTUNA_N_TRIALS_LGBM, show_progress_bar=True)
    lgbm_p = {**lgbm_study.best_params, "verbose": -1, "random_state": RANDOM_STATE}
    if "lr" in lgbm_p:
    ...
    def xgb_obj(trial):
        ...
        return cv_mape_score(global_df, final_features, cv_folds,lambda: xgb_module.XGBRegressor(**params), horizons=CV_HORIZONS)

    xgb_study = optuna.create_study(direction="minimize",sampler=optuna.samplers.TPESampler(seed=RANDOM_STATE))
    xgb_study.optimize(xgb_obj, n_trials=OPTUNA_N_TRIALS_XGB, show_progress_bar=True)
    xgb_p = {**xgb_study.best_params, "random_state": RANDOM_STATE, "verbosity": 0}
    if "lr" in xgb_p:
    ...
    def rf_obj(trial):
        ...
        return cv_mape_score(global_df, final_features, cv_folds,lambda: RandomForestRegressor(**params), horizons=CV_HORIZONS)
    ...
    rf_p = {**rf_study.best_params, "n_jobs": -1, "random_state": RANDOM_STATE}
    tuned_params["rf"] = rf_p
    with mlflow.start_run(run_name="rf_optuna_tuning"):
        mlflow.log_params(rf_p)
        mlflow.log_metric("best_cv_mape", rf_study.best_value)
        mlflow.set_tag("model_type", "RandomForest")
        mlflow.set_tag("n_trials", OPTUNA_N_TRIALS_RF)

    params_path = ML_MODELS_DIR / "tuned_params.json"
    with open(params_path, "w") as f:
        to_save = {k: {p: v for p, v in v.items() if p not in ("verbose", "verbosity")}
                   for k, v in tuned_params.items()}
        json.dump(to_save, f, ensure_ascii=False, indent=2)
    return tuned_params


def walk_forward_cv(global_df: pd.DataFrame,final_features: list,cv_folds: list,make_lgbm_fn, make_xgb_fn, make_rf_fn) -> pd.DataFrame:
    results = []
    for train_mask, test_mask, fold_num in cv_folds:
        ...
        forecasters = {
            ...
            "RandomForest": DirectForecaster(make_rf_fn,"RandomForest",CV_HORIZONS)}
        for fc in forecasters.values():
        ...
        for h in CV_HORIZONS:
            ...
            for name, fc in forecasters.items():
                ...
                results.append({"fold": fold_num, "horizon": h, "model": name,**compute_metrics(y_true, y_pred)})
...
def train_final_models(global_df: pd.DataFrame,final_features: list,make_lgbm_fn, make_xgb_fn, make_rf_fn) -> dict:
    ...
    rf_fc = DirectForecaster(make_rf_fn,"RandomForest",horizons)
    en_fc = DirectForecaster(make_elasticnet,"ElasticNet",horizons)
    lgbm_q10 = DirectForecaster(lambda: make_lgbm_quantile(0.1),"LGBM_Q10",horizons)
    lgbm_q90 = DirectForecaster(lambda: make_lgbm_quantile(0.9),"LGBM_Q90",horizons)
    lgbm_fc.fit(X_train, train_df)
    xgb_fc.fit(X_train, train_df)
    rf_fc.fit(X_train, train_df)
    ...
    rf_full = DirectForecaster(make_rf_fn,"RandomForest_full", horizons)
    lgbm_q10_f = DirectForecaster(lambda: make_lgbm_quantile(0.1),"LGBM_Q10_full", horizons)
    lgbm_q90_f = DirectForecaster(lambda: make_lgbm_quantile(0.9),"LGBM_Q90_full", horizons)

    lgbm_full.fit(X_full, global_df)
    xgb_full.fit(X_full, global_df)
    rf_full.fit(X_full, global_df)
    lgbm_q10_f.fit(X_full, global_df)
    lgbm_q90_f.fit(X_full, global_df)

    return {
        "lgbm": lgbm_fc, "xgb": xgb_fc, "rf": rf_fc, "en": en_fc,
        "lgbm_q10": lgbm_q10, "lgbm_q90": lgbm_q90,
        "X_train": X_train, "X_test": X_test,
        "X_tr_sc": X_tr_sc, "X_te_sc": X_te_sc,
        "train_df": train_df, "test_df": test_df,
        "scaler": scaler,
        "lgbm_full": lgbm_full, "xgb_full": xgb_full, "rf_full": rf_full,
        "lgbm_q10_full": lgbm_q10_f, "lgbm_q90_full": lgbm_q90_f}


def evaluate_on_test(trained: dict, final_features: list) -> pd.DataFrame:
    ...
    for (cat, ch), grp in test_df.groupby(["_category", "_channel"]):
        for h in CV_HORIZONS:
            ...
            for name, fc, x_te in [
                ...
                ("RandomForest", trained["rf"],   X_test),
                ("ElasticNet", trained["en"],   X_te_sc)]:
                y_pred = fc.predict(x_te.loc[vi], h)
                rows.append({"category": cat, "channel": ch,"model": name, "horizon": h,**compute_metrics(y_true, y_pred)})

            y_lo = np.expm1(trained["lgbm_q10"].predict_log(X_test.loc[vi], h))
            y_hi = np.expm1(trained["lgbm_q90"].predict_log(X_test.loc[vi], h))
            ci = coverage_and_width(y_true, y_lo, y_hi)
            rows.append({"category": cat, "channel": ch,
                          "model": "LGBM_CI", "horizon": h,
                          "mae": np.nan, "rmse": np.nan,
                          "mape": np.nan, "smape": np.nan, **ci})
...
def evaluate_h12(trained: dict, final_features: list) -> pd.DataFrame:
    test_df = trained["test_df"]
    train_df = trained["train_df"]
    X_train = trained["X_train"]
    rows = []
    model_items = [
        ("LightGBM", trained["lgbm"]),
        ("XGBoost", trained["xgb"]),
        ("RandomForest", trained["rf"])]
    for model_name, fc in model_items:
        for (cat, ch), grp_te in test_df.groupby(["_category", "_channel"]):
            ...
            for h, future_date in enumerate(te_dates, start=1):
                ...
                y_trues.append(float(np.expm1(fact_rows[TARGET_COL].values[0])))
            if not y_preds:
                continue
            m = compute_metrics(np.array(y_trues), np.array(y_preds))
            rows.append({"category": cat, "channel": ch, "model": model_name, **m})
...
def shap_analysis(trained: dict, final_features: list):
    ...
    plt.tight_layout()
    plt.savefig(ML_RESULTS_DIR / "shap_importance.png", dpi=130, bbox_inches="tight")
    plt.close()
    mean_abs = np.abs(shap_values.values).mean(axis=0)
    shap_df = pd.DataFrame({"feature": final_features,"mean_|shap|": mean_abs}).sort_values("mean_|shap|",
                                                                     ascending=False)
    shap_df.to_csv(ML_RESULTS_DIR / "shap_importance.csv", index=False)
    return shap_df


def generate_forecast(series_dict: dict,
    ...
    for (cat, ch), df_ in series_dict.items():
        ...
        last_date = s.index[-1]
        future_idx = pd.date_range(last_date + pd.DateOffset(months=1),periods=HORIZON, freq="MS")
        mask_s = (global_df["_category"] == cat) & (global_df["_channel"] == ch)
        ...
        for h in range(1, HORIZON + 1):
            ...
            y_lo_v = float(trained["lgbm_q10_full"].predict_log(x_h, h)[0])
            y_hi_v = float(trained["lgbm_q90_full"].predict_log(x_h, h)[0])
            if y_lo_v > y_hi_v:
...
def log_to_mlflow(cv_results: pd.DataFrame,
    ...
    model_specs = [
        ...
        ("XGBoost", "xgb",  "xgb"),
        ("RandomForest", "rf", "rf")]
    for model_name, param_key, fc_key in model_specs:
        with mlflow.start_run(run_name=f"{model_name.lower()}_final") as run:
            if tuned_params and param_key in tuned_params:
                mlflow.log_params(tuned_params[param_key])
            ...
            if not cv_m.empty:
                mlflow.log_metric("cv_mape_mean_h136", round(cv_m["mape"].mean(), 3))
            ...
            if not h12_m.empty:
                ...
                mlflow.log_metric("test_mape_h12_mean", round(h12_m["mape"].mean(), 3))
            ...
            for art in ["cv_results.csv", "test_metrics_ml.csv", "test_metrics_h12.csv"]:
                mlflow.log_artifact(str(ML_RESULTS_DIR / art))
            mlflow.log_artifact(str(meta_path))
            if (ML_RESULTS_DIR / "shap_importance.png").exists():
                mlflow.log_artifact(str(ML_RESULTS_DIR / "shap_importance.png"))
            ...
            with open(full_path, "wb") as f:
                pickle.dump(trained[f"{fc_key}_full"], f)
            mlflow.log_artifact(str(full_path))

            if not np.isnan(h12_median) and h12_median < best_mape:
...
def main():
    df_raw, series_dict = load_raw_data()
    lag_ranges = ccf_analysis(series_dict)
    global_df, feature_cols, cat_enc, ch_enc = build_dataset(series_dict, lag_ranges)
    cv_folds = make_cv_folds(global_df)
    final_features = feature_selection(global_df, feature_cols, cv_folds)
    tuned_params = tune_hyperparams(global_df, final_features, cv_folds)
    lgbm_p = tuned_params.get("lgbm", LGBM_DEFAULT_PARAMS)
    xgb_p = tuned_params.get("xgb", XGB_DEFAULT_PARAMS)
    rf_p = tuned_params.get("rf",RF_DEFAULT_PARAMS)
    make_lgbm_tuned = ModelFactory("lgbm", lgbm_p)
    make_xgb_tuned = ModelFactory("xgb", xgb_p)
    make_rf_tuned = ModelFactory("rf", rf_p)

    cv_results = walk_forward_cv(global_df, final_features, cv_folds,make_lgbm_tuned, make_xgb_tuned, make_rf_tuned)
    trained = train_final_models(global_df, final_features,make_lgbm_tuned, make_xgb_tuned, make_rf_tuned)
    test_metrics = evaluate_on_test(trained, final_features)
    test_h12 = evaluate_h12(trained, final_features)
    shap_df = shap_analysis(trained, final_features)
    future_fc = generate_forecast(series_dict, global_df, trained, final_features)

    log_to_mlflow(cv_results, test_metrics, test_h12,tuned_params, trained, final_features,
    ...
    with open(ML_MODELS_DIR / "ml_metadata.json", "w") as f:
        json.dump({
            ...
            "xgb_params": xgb_p,
            "rf_params": rf_p,
            "ccf_stationarity_fix": True}, f, ensure_ascii=False, indent=2)
