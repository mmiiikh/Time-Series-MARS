...
from src.config.settings import (
    ...
    ML_RESULTS_DIR)
from src.forecasting.ml_model import DirectForecaster, cv_mape_score
...
def _log_experiment(exp_name: str, model_name: str, params: dict,
    ...
    with mlflow.start_run(run_name=f"{exp_name}_{model_name.lower().replace(' ', '_')}"):
...
def experiment_clustering(series_dict: dict, global_df: pd.DataFrame,
                           ...
                           make_lgbm_fn, make_xgb_fn, make_rf_fn,
    ...
    cluster_df["cluster"] = KMeans(n_clusters=n_clusters, random_state=RANDOM_STATE, n_init=10).fit_predict(X_cl)

    cluster_map = {(r["category"], r["channel"]): int(r["cluster"]) for _, r in cluster_df.iterrows()}
    gdf["_cluster"] = gdf.apply(lambda r: cluster_map.get((r["_category"], r["_channel"]), -1), axis=1)

    train_df, test_df = _train_test_split(gdf)
    results = {}
    for model_name, make_fn in [("LightGBM", make_lgbm_fn), ("XGBoost", make_xgb_fn), ("RandomForest", make_rf_fn)]:
        rows_test = []
        for c in range(n_clusters):
            ...
            model_c = DirectForecaster(make_fn, f"Cluster_{c}", list(range(1, HORIZON+1)))
        ...
        for (cat, ch), grp in test_df.groupby(["_category", "_channel"]):
            for h in CV_HORIZONS:
                ...
                vi = valid[valid].index
                y_true = np.expm1(y_h.loc[vi].values)
                y_pred = global_fc.predict(test_df.loc[vi][final_features].fillna(0), h)
        ...
        results[model_name] = {"cluster_mape": clus_mape, "global_mape": global_mape}
...
def experiment_cross_series(global_df: pd.DataFrame,
                             ...
                             final_features: list,
                             cv_folds: list,
                             make_lgbm_fn, make_xgb_fn, make_rf_fn) -> dict:
    df = global_df.copy()
    ...
    for fold in range(N_FOLDS):
        ...
        if te_start < 2*SEASONAL_PERIOD:
            break
        new_folds.append((
            df_clean["_date"].isin(dates[:te_start]),
            df_clean["_date"].isin(dates[te_start:te_end]),
            fold+1))
    new_folds.reverse()
    results = {}
    for model_name, make_fn in [("LightGBM", make_lgbm_fn), ("XGBoost", make_xgb_fn), ("RandomForest", make_rf_fn)]:
        ...
        base_mape = cv_mape_score(df_clean, feats_ok_base, new_folds, make_fn, CV_HORIZONS)
        cross_mape = cv_mape_score(df_clean, feats_ok_cross, new_folds, make_fn, CV_HORIZONS)
        delta = cross_mape-base_mape

        _log_experiment(
            "exp3_cross_series", model_name,
            params={"new_features": str(new_feats), "eval": "cv h=1,3,6 mean"},
            metrics={
...
def experiment_horizon_specific(global_df: pd.DataFrame,final_features: list,cv_folds: list,
                                 make_lgbm_fn, make_xgb_fn, make_rf_fn) -> dict:
    ...
    for model_name, make_fn in [("LightGBM", make_lgbm_fn), ("XGBoost", make_xgb_fn), ("RandomForest", make_rf_fn)]:
        if model_name == "LightGBM":
            ...
            base_params = make_rf_fn().get_params()
            leaf_base = base_params.get("min_samples_leaf", 5)
            def make_short():
                p = {**base_params, "min_samples_leaf": max(2, leaf_base // 2)}
                return RandomForestRegressor(**p)
            def make_long():
                p = {**base_params, "min_samples_leaf": min(30, leaf_base * 2)}
                return RandomForestRegressor(**p)
        ...
        for train_mask, test_mask, fold_num in cv_folds:
            ...
            fc_single = DirectForecaster(make_fn, "Single", horizons_short + horizons_long)
            ...
            for h in eval_horizons:
                ...
                rows.append({"fold": fold_num, "horizon": h, "strategy": "Sep",**compute_metrics(y_true, y_sep)})
                rows.append({"fold": fold_num, "horizon": h, "strategy": "Single",**compute_metrics(y_true, y_sing)})
        ...
        _log_experiment(
            ...
            params={"short_horizons": str(horizons_short),
                    "long_horizons": str(horizons_long[:3]) + "...",
...
def experiment_ablation(global_df: pd.DataFrame,final_features: list,cv_folds: list,
                         make_lgbm_fn, make_xgb_fn, make_rf_fn) -> dict:
    from src.config.settings import EXOG_COLS

    groups = {
        "Все признаки (baseline)": final_features,
        "Без лагов экзогенных": [f for f in final_features if not any(col.replace(" ", "_").replace("/", "_") in f for col in EXOG_COLS)],
        "Без rolling/ewm": [f for f in final_features if "rolling" not in f and "ewm" not in f],
        "Без календарных": [f for f in final_features if not any(c in f for c in ["month", "quarter", "year", "is_q4", "is_summer"])],
        ...
        "Только лаги y + rolling": [f for f in final_features if f.startswith("y_lag") or "rolling" in f or "ewm" in f]}
    ...
    for model_name, make_fn in [("LightGBM", make_lgbm_fn), ("XGBoost", make_xgb_fn), ("RandomForest", make_rf_fn)]:
        ...
        for name, feats in groups.items():
            feats_ok = [f for f in feats if f in global_df.columns]
            if not feats_ok:
                continue
            mape = cv_mape_score(global_df, feats_ok, cv_folds, make_fn, CV_HORIZONS)
            if name == "Все признаки (baseline)":
                baseline = mape
            delta = round(mape - baseline, 2) if baseline is not None else 0.0
            model_rows.append({
                "Вариант": name, "N признаков": len(feats_ok),
                "CV MAPE": round(mape, 2), "дельта": delta})

        abl_df = pd.DataFrame(model_rows)
        abl_df.to_csv(ML_RESULTS_DIR / f"exp5_ablation_{model_name.lower().replace(' ', '_')}.csv",index=False)
        _log_experiment("exp5_ablation", model_name,params={"n_groups": len(groups), "eval": "cv h=1,3,6 mean"},
            metrics={
...
def experiment_direct_vs_recursive(global_df: pd.DataFrame,final_features: list,series_dict: dict,
                                    cv_folds: list,
                                    make_lgbm_fn, make_xgb_fn, make_rf_fn) -> dict:
    eval_horizons = CV_HORIZONS
    results = {}
    ...
    for model_name, make_fn in [("LightGBM", make_lgbm_fn), ("XGBoost", make_xgb_fn), ("RandomForest", make_rf_fn)]:
        ...
        for train_mask, test_mask, fold_num in cv_folds:
            ...
            y_h1_tr = train_df.groupby(["_category", "_channel"])[TARGET_COL].shift(-1)
            ...
            for h in eval_horizons:
                ...
                rows.append({"fold": fold_num, "horizon": h, "strategy": "Direct",**compute_metrics(y_true, y_direct)})

                y_rec_all = []
                for (cat, ch), grp in test_df.loc[idx].groupby(["_category", "_channel"]):
                    tr_series = train_df[
                    ...
                    for step in range(1, h+1):
                        ...
                        if "ewm_03" in final_features:
                            x_last["ewm_03"] = float(pd.Series(hist).ewm(alpha=0.3).mean().iloc[-1])
                        if "ewm_07" in final_features:
                            x_last["ewm_07"] = float(pd.Series(hist).ewm(alpha=0.7).mean().iloc[-1])
                        if step <= len(grp_idx):
                            x_src = X_test.loc[[grp_idx[min(step - 1, len(grp_idx) - 1)]]]
                            for cal in ["month", "month_sin", "month_cos","quarter", "quarter_sin", "quarter_cos","is_q4", "is_summer", "year"]:
                    ...
                    y_rec_all.extend([float(np.expm1(preds_log[-1]))] * len(grp_idx))

                y_rec = np.array(y_rec_all[:len(idx)])
                valid_rc = ~np.isnan(y_rec)
                if valid_rc.sum() > 0:
                    rows.append({"fold": fold_num, "horizon": h, "strategy": "Recursive",**compute_metrics(y_true[valid_rc], y_rec[valid_rc])})
        ...
        _log_experiment(
            ...
            params={"eval_horizons": str(eval_horizons), "eval": "cv h=1,3,6 mean"},
            metrics={
...
def experiment_global_vs_local(global_df: pd.DataFrame,final_features: list,make_lgbm_fn, make_xgb_fn, make_rf_fn) -> dict:
    ...
    for model_name, make_fn in [("LightGBM", make_lgbm_fn), ("XGBoost", make_xgb_fn), ("RandomForest", make_rf_fn)]:
        ...
        for cat, ch in test_df.groupby(["_category", "_channel"]).groups.keys():
            ...
            y_h1_s = tr_s.groupby(["_category", "_channel"])[TARGET_COL].shift(-1)
            ...
            for h in eval_h:
                ...
                rows.append({"category": cat, "channel": ch,"model": "Глобальная", "horizon": h,**compute_metrics(y_true, y_global)})
                if valid_s.sum() >= 15:
                    y_local = local_fc.predict(X_te_s.loc[vi], h)
                    rows.append({"category": cat, "channel": ch,"model": "Локальная", "horizon": h,**compute_metrics(y_true, y_local)})
        ...
        _log_experiment(
            ...
            params={"eval_horizons": str(eval_h), "eval": "test h=1,3,6 mean"},
            metrics={
                "global_mape": round(global_m, 3),
                "local_mape":  round(local_m, 3),
                "delta":       round(global_m - local_m, 3)},tags={"note": "test_based_not_cv"})
...
def experiment_stacking(global_df: pd.DataFrame,
    ...
    X_tr_sc = pd.DataFrame(scaler.fit_transform(X_train),columns=final_features, index=X_train.index)
    X_te_sc = pd.DataFrame(scaler.transform(X_test),columns=final_features, index=X_test.index)
    ...
    oof_rf = np.full(len(train_df), np.nan)
    ...
    for k in range(kfold):
        ...
        oof_rf[val_mask.values] = make_rf_fn().fit(X_tr_k, y_tr_k).predict(X_val_k)
        en_k = make_elasticnet().fit(X_tr_sc.loc[tr_mask], y_tr_k)
        oof_en[val_mask.values] = en_k.predict(X_tr_sc.loc[val_mask])

    oof_mask = ~(np.isnan(oof_lgbm)|np.isnan(oof_xgb)|np.isnan(oof_rf)|np.isnan(oof_en))
    if oof_mask.sum() < 20:
        return {}

    X_meta_train = np.column_stack([oof_lgbm[oof_mask], oof_xgb[oof_mask],oof_rf[oof_mask], oof_en[oof_mask]])
    ...
    rf_f = make_rf_fn().fit(X_train, y_train_log)
    ...
    y_rf_all = np.expm1(rf_f.predict(X_test))
    ...
    for (cat, ch), grp in test_df.groupby(["_category", "_channel"]):
        for h in CV_HORIZONS:
            ...
            for model_name, y_pred in [
                ...
                ("LightGBM", y_lgbm_all),
                ("XGBoost", y_xgb_all),
                ("RandomForest", y_rf_all)]:
                rows.append({"category": cat, "channel": ch,"model": model_name, "horizon": h,**compute_metrics(y_true, y_pred[pos])})
    ...
    rf_m = stack_df[stack_df["model"] == "RandomForest"]["mape"].mean()
    best_base = min(lgbm_m, xgb_m, rf_m)

    _log_experiment(
        "exp8_stacking", "LGBM+XGB+RF+EN→Ridge",
        params={"ridge_alpha": 1.0, "kfold_oof": kfold,
                "base_learners": "LGBM+XGB+RF+EN",
                "eval": "test h=1,3,6 mean"},
        metrics={
            "stack_mape": round(stack_m, 3),
            "lgbm_mape":round(lgbm_m, 3),
            "xgb_mape": round(xgb_m, 3),
            "rf_mape": round(rf_m,3),
            "delta_vs_best": round(stack_m - best_base, 3),
            "ridge_coef_lgbm": round(float(meta.coef_[0]), 3),
            "ridge_coef_xgb": round(float(meta.coef_[1]), 3),
            "ridge_coef_rf": round(float(meta.coef_[2]), 3),
            "ridge_coef_en": round(float(meta.coef_[3]), 3)},
        tags={"note": "test_based_not_cv"})
    return {"stack_mape": stack_m, "lgbm_mape": lgbm_m,"xgb_mape":   xgb_m, "rf_mape":   rf_m}
...
def run_all_experiments(pipeline_artifacts: dict):
    ...
    make_rf_fn = pipeline_artifacts["make_rf_tuned"]
    ...
    base_lgbm = cv_mape_score(global_df, final_features, cv_folds, make_lgbm_fn, CV_HORIZONS)
    base_xgb = cv_mape_score(global_df, final_features, cv_folds, make_xgb_fn, CV_HORIZONS)
    base_rf = cv_mape_score(global_df, final_features, cv_folds, make_rf_fn, CV_HORIZONS)

    kw = dict(make_lgbm_fn=make_lgbm_fn,make_xgb_fn=make_xgb_fn,make_rf_fn=make_rf_fn)
    all_results = {}

    try:
        all_results["clustering"] = experiment_clustering(series_dict, global_df, final_features, cv_folds, **kw)
    except Exception as e:
        print(f"[WARN] clustering: {e}")

    try:
        all_results["cross_series"] = experiment_cross_series(global_df, feature_cols, final_features, cv_folds, **kw)
    except Exception as e:
        print(f"[WARN] cross_series: {e}")

    try:
        all_results["horizon_specific"] = experiment_horizon_specific(global_df, final_features, cv_folds, **kw)
    except Exception as e:
        print(f"[WARN] horizon_specific: {e}")

    try:
        all_results["ablation"] = experiment_ablation(global_df, final_features, cv_folds, **kw)
    except Exception as e:
        print(f"[WARN] ablation: {e}")

    try:
        all_results["direct_vs_recursive"] = experiment_direct_vs_recursive(global_df, final_features, series_dict, cv_folds, **kw)
    except Exception as e:
        print(f"[WARN] direct_vs_recursive: {e}")

    try:
        all_results["global_vs_local"] = experiment_global_vs_local(global_df, final_features, **kw)
    except Exception as e:
        print(f"[WARN] global_vs_local: {e}")

    try:
        all_results["stacking"] = experiment_stacking(global_df, final_features, **kw)
    except Exception as e:
        print(f"[WARN] stacking: {e}")
