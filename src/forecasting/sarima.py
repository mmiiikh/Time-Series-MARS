...
from src.config.settings import TARGET_COL, EXOG_CANDIDATES, SEASONAL_PERIOD, RANDOM_STATE,SARIMA_DIR, RESULTS_DIR
...
def _get_exog_cols(df: pd.DataFrame) -> list:
    return [c for c in SARIMAX_EXOG_CANDIDATES if c in df.columns and df[c].notna().sum() > 10]
...
def _forecast_exog_via_sarima(exog_series: pd.Series, horizon: int) -> pd.Series:
    future_idx = pd.date_range(exog_series.index[-1] + pd.DateOffset(months=1), periods=horizon, freq="MS")
    try:
        m = auto_arima(
            exog_series.dropna(), m=SEASONAL_PERIOD, seasonal=True,
...
def _analyze_exog_correlation(series_dict: dict) -> pd.DataFrame:
    ...
    for (cat, ch),df in series_dict.items():
        ...
        for col in _get_exog_cols(df):
            ...
            mi= mutual_info_regression(x[common].values.reshape(-1, 1), y[common].values,random_state=RANDOM_STATE)[0]
            rows.append({"category": cat, "channel": ch,"exog_col": col, "pearson_r": round(r,4),"mutual_info": round(mi,4)})
    ...
    for (cat, ch),df in series_dict.items():
        ...
        for col in _get_exog_cols(df):
            ...
            try:
                ...
                rows.append({"category": cat, "channel": ch, "exog_col": col,"granger_min_p": round(min_p, 4),"granger_significant": min_p < 0.05})
            except Exception:
                pass
    return pd.DataFrame(rows)
...
def _rf_importance(series_dict: dict, n_lags: int = 3) -> pd.DataFrame:
    ...
    for (cat, ch),df in series_dict.items():
        ...
        for col in exog_cols:
            ...
            rows.append({"category": cat, "channel": ch,"exog_col": col, "rf_importance": round(float(total), 4)})
    return pd.DataFrame(rows)
...
def select_exog_variables(series_dict: dict,
    ...
    for pair in sorted(all_pairs):
        ...
        for var in sorted(all_vars):
            ...
            if not corr_df.empty:
                m = corr_df[(corr_df["category"] == cat) & (corr_df["channel"] == ch)&(corr_df["exog_col"] == var)]["pearson_r"].abs()
                if len(m):
                    pearson_val = m.values[0]
                    if pearson_val >= pearson_thresh:
                        score+=1
            if not gc_df.empty:
                m = gc_df[(gc_df["category"] == cat) & (gc_df["channel"] == ch)&(gc_df["exog_col"] == var)]["granger_min_p"]
                if len(m) and m.values[0] < granger_p_thresh:
                    score+=1
            if not rf_df.empty:
                m = rf_df[(rf_df["category"] == cat) & (rf_df["channel"] == ch)&(rf_df["exog_col"] == var)]["rf_importance"]
        ...
        # Frequency убрана из кандидатов — нет обязательных переменных
        ...
        if len(after_stage3)>=2 and pair in series_dict:
            ...
            avail = [c for c in after_stage3 if c in df_s.columns and df_s.loc[y_idx, c].notna().all()]
            if len(avail)>=2:
    ...
    print(f"Кандидаты SARIMAX: {SARIMAX_EXOG_CANDIDATES}")
...
def find_global_order(series_dict: dict, per_series_exog: dict, n_sample: int=5) -> dict:
    ...
    order = {k: mode(v) for k, v in zip(["p", "d", "q", "P", "D", "Q"],[p_list, d_list, q_list, P_list, D_list, Q_list])}
    print(f"auto_arima:"
...
def train_all_models(series_dict: dict, per_series_exog: dict, global_order: dict, use_individual_order: bool = True, test_size: int = 12) -> dict:
    results = {}
    ...
    for i,(key, df_) in enumerate(series_dict.items(),1):
        ...
        if candidates:
            avail = [c for c in candidates if c in df_.columns and df_.loc[y.index, c].notna().all()]
            if avail:
        ...
        if use_individual_order:
            try:
                ...
                seasonal_order = (global_order["P"], global_order["D"],global_order["Q"], SEASONAL_PERIOD)
        else:
            order = (global_order["p"], global_order["d"], global_order["q"])
            seasonal_order = (global_order["P"], global_order["D"],global_order["Q"], SEASONAL_PERIOD)
...
def train_seasonal_naive(series_dict: dict, test_size: int = 12) -> dict:
    ...
    for key,df_ in series_dict.items():
        ...
        fc = np.array([y_train.iloc[-(SEASONAL_PERIOD - h % SEASONAL_PERIOD)] if len(y_train) >= SEASONAL_PERIOD else y_train.iloc[-1] for h in range(test_size)])
...
def train_sarima_no_exog(series_dict: dict, global_order: dict, test_size: int = 12) -> dict:
    empty_exog = {key: [] for key in series_dict}
    return train_all_models(series_dict, empty_exog, global_order,use_individual_order=False, test_size=test_size)
...
def train_arima(series_dict: dict, test_size: int = 12) -> dict:
    ...
    for i,(key, df_) in enumerate(series_dict.items(), 1):
        ...
        results[key] = {
            ...
            "y_train": y_train,
            "y_test": y_test,
            "forecast_test":  pd.Series(forecast_test.values, index=y_test.index),
            "metrics": metrics,
            "aic":fitted.aic}
...
def train_prophet(series_dict: dict, test_size: int = 12) -> dict:
    results = {}
    #total   = len(series_dict)
    covid_changepoints = [pd.Timestamp("2020-03-01"),pd.Timestamp("2020-06-01")]
    for i,(key, df_) in enumerate(series_dict.items(),1):
        cat,ch = key
        y = df_[TARGET_COL].dropna()
        if len(y) < SEASONAL_PERIOD+test_size:
        ...
        y_train = y.iloc[:-test_size]
        y_test  = y.iloc[-test_size:]
        df_prophet = pd.DataFrame({"ds": y_train.index.to_pydatetime(),"y":  y_train.values.astype(float)})
        seasonality_mode = ("multiplicative" if (y_train > 0).all() else "additive")
        train_changepoints = [cp for cp in covid_changepoints if y_train.index.min() <= cp <= y_train.index.max()]
        try:
            model = _Prophet(
                ...
                uncertainty_samples=1000,   # для 80% CI
                changepoints=train_changepoints if train_changepoints else None)
            model.fit(df_prophet)
        except Exception as e:
            continue

        future_df = pd.DataFrame({"ds": y_test.index.to_pydatetime()})
        forecast_prophet = model.predict(future_df)
        yhat = np.maximum(forecast_prophet["yhat"].values,0.0)
        yhat_lo = np.maximum(forecast_prophet["yhat_lower"].values,0.0)
        yhat_hi = forecast_prophet["yhat_upper"].values
        forecast_test = pd.Series(yhat, index=y_test.index)
        metrics = compute_metrics(y_test.values, yhat)
        metrics["lb_p"] = np.nan
        metrics["resid_ok"] = False
        results[key] = {
            "model": model,
            ...
            "forecast_lower":pd.Series(yhat_lo, index=y_test.index),
            "forecast_upper": pd.Series(yhat_hi, index=y_test.index),
            "metrics": metrics,
            "aic": np.nan,
            "seasonality_mode": seasonality_mode}
...
def select_best_and_save(
    results_sarimax: dict,
    results_sarima:  dict,
    results_naive:   dict,
    results_arima:   dict=None,
    results_prophet: dict=None,
    metric:          str="mape") -> dict:

    results_arima = results_arima or {}
    results_prophet = results_prophet or {}
    all_keys = (
        set(results_sarimax)
        | set(results_sarima)
        | set(results_naive)
    ...
    for key in sorted(all_keys):
        candidates = {"Naive":results_naive.get(key), "ARIMA":results_arima.get(key),"SARIMA":results_sarima.get(key),"SARIMAX": results_sarimax.get(key),"Prophet":results_prophet.get(key)}
        candidates = {k: v for k,v in candidates.items() if v is not None}
        if not candidates:
            continue

        best_label = min(candidates,key=lambda k: candidates[k]["metrics"].get(metric, np.inf) or np.inf)
        best_res = candidates[best_label]
        safe_name  = f"{key[0]}_{key[1]}".replace(" ", "_").replace("/", "-")
        model_file = str(SARIMA_DIR / f"{safe_name}.pkl")
        if best_res.get("model") is not None:
            if best_label == "Prophet":
                with open(model_file, "wb") as f:
                    pickle.dump({
                        ...
                        "seasonal_order": (0,0,0,SEASONAL_PERIOD),
                        "exog_cols":[],
                        "seasonality_mode": best_res.get("seasonality_mode", "multiplicative")}, f)
            else:
                with open(model_file, "wb") as f:
                    pickle.dump({
                        "fitted_model": best_res["model"],
                        ...
                        "exog_cols":best_res["exog_cols"]}, f)
        else:
            model_file = None
        ...
        all_mapes = {k: round(float(v["metrics"].get(metric) or np.inf), 2) for k, v in candidates.items()}
        mape_str = "  ".join(f"{k}={v:.1f}%" for k, v in all_mapes.items())
...
def compare_all_models(
    ...
    all_keys = (
        ...
        | set(results_arima)
        | set(results_prophet))
    all_labels = ["Naive", "ARIMA", "SARIMA", "SARIMAX", "Prophet"]
    all_dicts  = {
        "Naive": results_naive,
        "ARIMA": results_arima,
    ...
    for key in sorted(all_keys):
        ...
        avail = {lb: row[f"{lb} MAPE,%"] for lb in all_labels if row.get(f"{lb} MAPE,%") is not None}
        row["Лучшая"] = min(avail, key=avail.get) if avail else "—"
        rows.append(row)
    ...
    if save_csv:
        out = RESULTS_DIR / "part1" / "sarima_all_models_comparison.csv"
...
def load_manifest() -> dict:
    if not MANIFEST_PATH.exists():
        raise FileNotFoundError(f"Манифест не найден: {MANIFEST_PATH}. Запусти обучение через /train.")
    with open(MANIFEST_PATH, "r", encoding="utf-8") as f:
        return json.load(f)


def forecast_from_manifest(series_dict: dict, category: str, channel: str, horizon: int=12,exog_df: pd.DataFrame = None) -> dict:
    ...
    df_ = series_dict[key]
    y = df_[TARGET_COL].dropna()
    future_idx = pd.date_range(y.index[-1] + pd.DateOffset(months=1), periods=horizon, freq="MS")
    if best_model=="Naive":
        fc = np.array([y.iloc[-(SEASONAL_PERIOD-h%SEASONAL_PERIOD)] for h in range(horizon)])
    ...
    model_type_pk = saved.get("model_type", best_model)
    if model_type_pk == "Prophet":
        if not PROPHET_AVAILABLE:
            raise RuntimeError("Prophet не установлен: pip install prophet")
        future_df = pd.DataFrame({"ds": future_idx.to_pydatetime()})
        forecast_p = fitted.predict(future_df)
        fc = np.maximum(forecast_p["yhat"].values,0.0)
        lo = np.maximum(forecast_p["yhat_lower"].values,0.0)
        hi = forecast_p["yhat_upper"].values
        return _build_response(category, channel, "Prophet",f"Prophet (yearly seasonality)",future_idx, fc, lower=lo, upper=hi, info=info)
    ...
    if exog_cols:
        ...
        if exog_df is not None:
            ...
            proj = {col: _forecast_exog_via_sarima(exog_hist[col].dropna(), horizon) for col in exog_cols}
            exog_future = pd.DataFrame(proj, index=future_idx)
    ...
    strategy = best_model+("(авто экзог.)" if exog_cols and exog_df is None else "(план экзог.)" if exog_cols else "")

    return _build_response(
        category, channel, strategy,
        f"{best_model}{order}x{seasonal_order[:3]}[{SEASONAL_PERIOD}]",
...
def _build_response(category, channel, model_type, model_spec,
    ...
    return {
        ...
        "forecast": [{"date":     str(d.date()) if hasattr(d, "date") else str(d), "forecast": round(float(f), 2), "lower_80": round(float(lo), 2), "upper_80": round(float(hi), 2)} for d, f, lo, hi in zip(dates, forecast, lower, upper)]}