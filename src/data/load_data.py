import numpy as np
import pandas as pd
from src.config.settings import TARGET_COL, DATE_COL, CATEGORY_COL, CHANNEL_COL, DATA_PATH
...
def create_series_dict(df: pd.DataFrame) -> dict:
    ...
    for (cat,ch), grp in df.groupby([CATEGORY_COL, CHANNEL_COL]):
        ...
        grp[num_cols] = grp[num_cols].interpolate(method="linear", limit_direction="both")
