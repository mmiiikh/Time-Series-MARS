...
def get_stl_decomposition(series_dict: dict, category: str, channel: str) -> dict:
    ...
    return {
        ...
        "metrics": {
            ...
            "trend_slope_mo": round(float((res.trend.iloc[-1] - res.trend.iloc[0]) / len(res.trend)), 0)}}
...
def get_acf_pacf(series_dict: dict, category: str, channel: str, n_lags: int =36, diff: str = "original") -> dict:
...
def analyze_ccf(series_dict: dict, max_lag: int = 12,
    ...
    summary = ccf_df.groupby("exog_col").agg(
        ...
        pct_significant=("best_ccf", lambda x: (np.abs(x) > 0.3).mean())).round(3).sort_values("mean_best_ccf", ascending=False)
...
def plot_ccf_heatmap(ccf_df: pd.DataFrame, max_lag: int = 8,
    ...
    sns.heatmap(pivot, annot=True, fmt=".2f", cmap="YlOrRd",vmin=0, vmax=1, linewidths=0.5, ax=ax)
...
def build_features_single(df_: pd.DataFrame, key: tuple,
    ...
    feat["rolling_pos_6"] = ((s_log_sh - feat["rolling_min_6"]) /(feat["rolling_max_6"] - feat["rolling_min_6"] + 1e-8))
    ...
    feat["covid"] = ((s.index >= COVID_START) & (s.index <= COVID_END)).astype(int)
    feat["post_covid"] = ((s.index > COVID_END) & (s.index <= "2021-12-01")).astype(int)
...
def build_global_dataset(series_dict: dict,
    ...
    feature_cols = [c for c in global_df.columns if c not in [TARGET_COL, "_category", "_channel", "_date"]]
    lag_cols = [c for c in feature_cols if "lag" in c or "rolling" in c]
    thresh = int(len(lag_cols) * 0.7)
    global_df = global_df.dropna(subset=lag_cols, thresh=thresh).reset_index(drop=True)
...
def make_cv_folds(global_df: pd.DataFrame,
    ...
    for fold in range(n_folds):
        ...
        folds.append((global_df["_date"].isin(train_dates),global_df["_date"].isin(test_dates),fold+1))
    folds.reverse()
